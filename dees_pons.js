...
class dees_Pons {
    ...
    async displayName() {
        ...
        if (locale.indexOf('CN') != -1) return 'Pons德西词典';
        if (locale.indexOf('TW') != -1) return 'Pons德西词典';
        return 'Pons.com DE->ES Dictionary';
    }
    ...
    async findTerm(word) {
        this.word = word;
        return await this.findPons(word);
    }
    ...
    async findPons(word) {
        ...
        let base = 'https://de.pons.com/übersetzung?l=dees&q=';
        ...
        let contents = doc.querySelectorAll('.results-container .target') || [];
